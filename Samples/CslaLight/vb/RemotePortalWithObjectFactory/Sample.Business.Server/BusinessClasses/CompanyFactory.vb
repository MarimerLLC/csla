...
Public Class CompanyFactory
  ...
  Private Function GetCompany(ByVal criteria As SingleCriteria(Of Company, Integer)) As Company
    Dim returnValue As Company = Nothing
    Using connection As New SqlConnection(DataConnection.ConnectionString)
      ...
      Using command As New SqlCommand("GetCompany", connection)
        ...
        Using reader As New Csla.Data.SafeDataReader(command.ExecuteReader())
          If reader.Read() Then
            returnValue = Company.LoadCompany(reader.GetInt32("CompanyID"), reader.GetString("CompanyName"), reader.GetSmartDate("DateAdded"))
            MarkOld(returnValue)
          End If
        End Using
      End Using
