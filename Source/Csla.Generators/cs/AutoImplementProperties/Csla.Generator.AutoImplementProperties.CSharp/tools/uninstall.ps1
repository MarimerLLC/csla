...
{
    # Uninstall the language agnostic source generators.
    if (Test-Path $generatorsPath)
    {
        foreach ($generatorFilePath in Get-ChildItem $generatorsPath -Filter *.dll)
        {
            ...
            {
                $project.Object.AnalyzerReferences.Remove($generatorFilePath.FullName)
            }
        }
    }
}
...
foreach($generatorsPath in $generatorsPaths)
{
    # Uninstall language specific source generators.
