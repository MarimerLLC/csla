...
namespace Csla.Test.ChildChanged
{
  ...
  public class OptimizeChildChangedTests
  {
    ...
    private void CheckBottomDepth(List<EventDetail> list, int depth)
    {
      ...
      Assert.AreEqual(2, list.RemoveAll(ev => ev.BO == enumBO.SimpleBO && ev.Depth == depth && ev.Event == enumEvent.PropertyChanged && ev.PropertyName == "IsDirty"));
      ...
      Assert.AreEqual(2, list.RemoveAll(ev => ev.BO == enumBO.SimpleBO && ev.Depth == depth && ev.Event == enumEvent.PropertyChanged && ev.PropertyName == "IsValid"));
      ...
      Assert.AreEqual(3, list.RemoveAll(ev => ev.BO == enumBO.SimpleBO && ev.Depth == depth && ev.Event == enumEvent.PropertyChanged && ev.PropertyName == "IsSavable"));
