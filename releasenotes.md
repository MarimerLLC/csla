...
**Test host and `AddCslaTesting` API** ([#4883](https://github.com/MarimerLLC/csla/issues/4883))

`Csla.Testing` now provides a supported way to stand up CSLA in a unit test. `CslaTestHost` is a self-contained, disposable host that owns a configured service provider and hands back an `ApplicationContext`, an `IDataPortal<T>`, or an `IChildDataPortal<T>`; a fluent builder configures CSLA options, registers services such as fakes for your own dependencies, and sets the principal the test runs as. For a test that owns its own `IServiceCollection`, `AddCslaTesting` adds just the supporting services CSLA needs — a hosting environment, logging, and a context manager — and is purely additive, leaving the `AddCsla` call to you. The principal is configured declaratively, so it works through either entry point, and the context manager holds its state per async flow rather than in static fields, so tests are not forced to run serially. Neither API depends on a test framework. See [Csla.Testing](docs/Csla-Testing.md) for details.
