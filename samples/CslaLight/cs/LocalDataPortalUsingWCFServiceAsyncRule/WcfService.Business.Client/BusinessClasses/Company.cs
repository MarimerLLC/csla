...
namespace WcfService.Business.Client
{
  ...
  public class Company : BusinessBase<Company>
  {
    ...
    private AsyncValidationRuleContext _context;
    ...
    private static void IsDuplicateName(AsyncValidationRuleContext context)
    {
      ...
      client.IsDuplicateNameCompanyCompleted -= DuplicateCheckCompleted;
      client.IsDuplicateNameCompanyCompleted += DuplicateCheckCompleted;
      client.IsDuplicateNameCompanyAsync(
        Convert.ToInt32(context.PropertyValues["CompanyId"]), 
        Convert.ToString(context.PropertyValues["CompanyName"]), 
        context);
    }

    private static void DuplicateCheckCompleted(object sender, CompanyServiceReference.IsDuplicateNameCompanyCompletedEventArgs e)
    {
      AsyncValidationRuleContext context = (AsyncValidationRuleContext)e.UserState;
      if (e.Result)
      {
