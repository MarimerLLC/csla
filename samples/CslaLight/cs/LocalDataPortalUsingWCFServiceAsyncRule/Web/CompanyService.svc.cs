...
namespace Web
{
	public class CompanyService : ICompanyService
	{
	  ...
	  public bool IsDuplicateNameCompany(int companyId, string companyName)
	  {
		...
		using (SqlConnection connection = new SqlConnection(DataConnection.ConnectionString))
		{
		  ...
		  using (SqlCommand command = new SqlCommand("IsDuplicateCompany", connection))
		  {
			...
			SqlParameter isDuplicateParameter = new SqlParameter("@isDuplicate", false);
			isDuplicateParameter.Direction = System.Data.ParameterDirection.Output;
			command.Parameters.Add(isDuplicateParameter);
			command.ExecuteNonQuery();
			returnValue = System.Convert.ToBoolean(isDuplicateParameter.Value);
		  }
		  connection.Close();
		}
		return returnValue;
