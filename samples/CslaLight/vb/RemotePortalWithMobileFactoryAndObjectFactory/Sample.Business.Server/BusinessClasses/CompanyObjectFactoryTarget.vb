...
<Serializable(), ObjectFactory("Sample.Business.CompanyObjectFactory, Sample.Business", "CreateCompany", "GetCompany", "SaveCompany", "")> _
Public Class CompanyObjectFactoryTarget
  Inherits BusinessBase(Of CompanyObjectFactoryTarget)
...
#End If
  ...
  Protected Overrides Sub AddBusinessRules()
    ...
    ValidationRules.AddRule(Of CompanyObjectFactoryTarget)(AddressOf IsDateValid, DateAddedProperty)

  End Sub

  Private Shared Function IsDateValid(ByVal target As CompanyObjectFactoryTarget, ByVal e As RuleArgs) As Boolean
  ...
  Friend Shared Function LoadCompany(ByVal companyId As Integer, ByVal companyName As String, ByVal dateAdded As SmartDate) As CompanyObjectFactoryTarget
    Dim newCompany As New CompanyObjectFactoryTarget
  ...
  Friend Shared Function NewCompany() As CompanyObjectFactoryTarget
    Dim aCompany As New CompanyObjectFactoryTarget
    aCompany.ValidationRules.CheckRules()
    Return aCompany
  End Function

  Friend Shared Function CloneCompany(ByVal source As Company) As CompanyObjectFactoryTarget
    Dim returnValue As New CompanyObjectFactoryTarget
    With returnValue
      .LoadProperty(CompanyIdProperty, source.CompanyId)
      .LoadProperty(CompanyNameProperty, source.CompanyName)
      .LoadProperty(DateAddedProperty, New SmartDate(source.DateAdded))
      .MarkOld()
      If source.IsNew Then
        .MarkNew()
      End If
      If source.IsDirty Then
        .MarkDirty()
      End If
      If source.IsDeleted Then
        .MarkDeleted()
      End If
